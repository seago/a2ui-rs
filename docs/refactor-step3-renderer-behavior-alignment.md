...
3. TUI Button/子组件渲染区域重叠导致帧缓冲断言不可靠（既有布局 quirk），D8 的 borderless 断言改为对 `button_display` 纯函数的单测。**quirk 本身已于后续 commit 修复**：Button 的 child 已被消费为 label、不再作为独立 widget 重复渲染（对齐其余三家）；Card 的 child 渲染在标题行下方。
