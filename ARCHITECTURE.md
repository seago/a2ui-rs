...
| `a2ui-renderer-web` | Web 渲染器实现（WASM 或服务端渲染） | `leptos`/`dioxus` 或 `axum`, `a2ui-renderer` |
...
**WebUI（浏览器界面）**
- 两种路线：
  - **WASM 路线**：Rust 编译为 WASM，在浏览器中运行渲染器，通过 JS bridge 与 DOM 交互
  - **服务端渲染路线**：Rust 后端渲染组件树为 HTML / Web Component，通过 WebSocket / SSE 推送到前端
- `Image` / `Video` / `AudioPlayer` 在 Web 端有原生 HTML 标签支持，渲染质量最高
- `Icon` 映射为 SVG icon font
- 适合与 AG-UI 的 Web 前端集成、MCP 客户端
