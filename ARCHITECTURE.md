...
| `a2ui-renderer` | `Renderer` trait、**`RendererCore`（四渲染器共享的协议状态 + 消息处理流水线，接入生命周期状态机，经 `CoreEffects` 通知平台缓存失效）**、组件森林、数据绑定引擎、路径解析、函数调度、Catalog 注册表、DependencyGraph、SurfaceLru、CustomComponentRegistry | `a2ui-core` |
