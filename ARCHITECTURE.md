...
└─────────────┘     └──────────────────┘     └────────┬─────────┘
                                    ...
                                    ▼                 ▼                 ▼
                              ┌──────────┐     ┌──────────┐     ┌──────────┐
                              │ TUI 渲染器│     │ GUI 渲染器│     │ WebUI 渲染器│
                              │ (ratatui)│     │ (egui/tao)│    │ (WASM/DOM) │
                              └──────────┘     └──────────┘     └──────────┘
                                    │                 │                 │
                                    ▼                 ▼                 ▼
                              终端界面            桌面窗口          浏览器页面
...
协议与渲染实现完全分离。Transport Layer 将 JSON 消息流送入 Protocol Core，Protocol Core 维护 Surface 状态并调用 Renderer trait，各平台 crate 负责实际渲染。
...
| `a2ui-transport` | 传输层抽象 trait + AG-UI / A2A / MCP / WebSocket / SSE 绑定 | `tokio`, `async-trait` |
| `a2ui-renderer` | `Renderer` trait、组件树管理、数据绑定引擎、路径解析、函数调度、Catalog 注册表 | `a2ui-core` |
| `a2ui-renderer-tui` | TUI 渲染器实现 | `ratatui`, `crossterm`, `a2ui-renderer` |
| `a2ui-renderer-gui` | GUI 渲染器实现 | `egui`, `tao`, `a2ui-renderer` |
| `a2ui-renderer-web` | Web 渲染器实现（静态派：服务端把组件树渲染为 HTML 字符串，非交互）；交互派为独立的 TS/React 前端项目，不在本 workspace | `a2ui-core`, `a2ui-renderer`, `serde_json` |
...
- 键盘事件（方向键 / Enter / Tab）映射为 `action` 消息，焦点管理通过本地 `registeredFunctions` 实现
- 优势：无依赖外部服务，适合本地 Agent 直连（STDIN/STDOUT 或 WebSocket）
- 约束：终端宽度有限，需做响应式裁剪；色彩受终端能力限制；无原生滚动，需要虚拟滚动

**GUI（桌面界面）**
- 基于 egui 即时模式 GUI，每帧根据 A2UI 组件树重建 egui widget
- 所有 18 个 Basic Catalog 组件均可完整实现（桌面环境无显示限制）
- 系统原生窗口、菜单、文件选择器通过 tao 提供
...
**交互派（TS/React 前端渲染器）设计要点**
- 浏览器通过 WebSocket / SSE 收 A2UI 消息流（`createSurface` / `updateComponents` / `updateDataModel` …），在前端维护组件树 + Data Model + 路径解析 + 响应性
- 采用**「协议核心 + 可插拔 ComponentKit」两层**结构：核心层遍历组件树并只写一次；每个 `ComponentKit` 把 18 个 A2UI 组件类型映射到某套 React 组件库（shadcn 为首个 kit，可切换到 MUI / Ant Design 等），切库只换 kit、不动核心层与状态
...
- 依赖 transport 层提供一个 **WS 服务端**（Agent 向浏览器推送 `ServerEnvelope`、接收 `ClientEnvelope`）；现有 `WebSocketTransport` 仅为客户端连接实现，服务端需另行补充
...
4. **服务端同步**：本地变更**不**自动发网络请求；只有 action 触发时才在 context 中附带修改后的数据
...
```json
{
  ...
  "action": {
    "name": "string",
    "surfaceId": "string",
    "sourceComponentId": "string (可选)",
    "context": { /* 动态数据 */ },
    "wantResponse": false,
    "responsePath": "JSON Pointer (可选)",
...
pub trait Renderer {
    /// 创建新的 Surface，返回句柄
    fn create_surface(&mut self, msg: CreateSurface) -> Result<SurfaceHandle, RendererError>;
    /// 向指定 Surface 添加或更新组件
    fn update_components(&mut self, msg: UpdateComponents) -> Result<(), RendererError>;
    /// 更新指定 Surface 的 Data Model
    fn update_data_model(&mut self, msg: UpdateDataModel) -> Result<(), RendererError>;
    /// 销毁指定 Surface
    fn delete_surface(&mut self, msg: DeleteSurface) -> Result<(), RendererError>;
    /// 处理服务端对 action 的响应
    fn action_response(&mut self, msg: ActionResponse) -> Result<(), RendererError>;
    /// 执行服务端请求的客户端函数
    fn call_function(&mut self, msg: CallFunction) -> Result<FunctionResponse, RendererError>;
    /// 渲染当前帧（各平台自行实现）
    fn render(&mut self) -> Result<(), RendererError>;

    /// 处理用户交互，生成 action 消息
    fn handle_user_event(&mut self, event: UserEvent) -> Result<Option<ActionMessage>, RendererError>;
}
```
...
- `Pending`：已收到 createSurface 但尚未完全初始化
- `Active`：正常状态，可接收 updateComponents / updateDataModel
- `Deleted`：已销毁，不能再接收消息
...
当 Surface 的 `sendDataModel: true` 时：

```rust
pub struct ClientDataModelPayload {
...
- 客户端在发送 `action` 或 `functionResponse` 时，通过 transport 的 metadata 机制附带此 payload
- 数据仅发送给创建该 Surface 的服务端，不泄露给其他 agent
